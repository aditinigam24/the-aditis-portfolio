...
import { AppError, BadRequestError, ValidationError, NotFoundError, ServiceUnavailableError } from "./errors.ts";
...
// ============ EMAIL ENDPOINT ============
app.post("/api/send-email", async (req: Request, res: Response, next: NextFunction) => {
  try {
...
// ============ GROQ API ENDPOINT ============
app.post("/api/jerry", async (req: Request, res: Response, next: NextFunction) => {
  try {
...
// ============ LOCAL JERRY BRAIN ============
function getJerryReply(message: string): string {
...
async function getGroqReply(userMessage: string): Promise<string> {
  try {
